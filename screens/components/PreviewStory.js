...
class PreviewStory extends Component {
  ...
  draw_progres = (pos, size, dim) =>{
    let len = (dim.width - 30) / size;
    let dummy = [];
    ...
    return(
      <View style={{flex : 1, marginLeft : 10, marginRight : 10, margin : 5, flexDirection : 'row'}}>
        {
          dummy.map((value, index)=><View style={{height : 4, marginLeft : 2, marginRight : 2, width : len, borderRadius : 5, backgroundColor : value === 0 ? 'rgba(0, 0, 0, 0.5)' : 'rgba(255, 255, 255, 0.5)'}} key={index}/>)
        }
      </View>
    );
  }
  ...
  render() {
    ...
    return(
      <Swiper
        ...
        scrollEnabled = {false}
        prevButton = { <Text style={{width : dim.width / 2 - 10, height : dim.height - 20, color : 'transparent', marginBottom : 0}}>‹</Text> }
        nextButton = { <Text style={{width : dim.width / 2 - 10, height : dim.height - 20, color : 'transparent', marginBottom : 0}}>›</Text>}
          ...
          Object.entries(data).map((obj, index)=>{
            ...
            return(<View style={{flex : 1, backgroundColor : '#000'}} key={index}>
              ...
              <View style ={{position : 'absolute', top : 0}}>
                {this.draw_progres(this.state.current, size, dim)}
              </View>
            </View>);
          })
        }
